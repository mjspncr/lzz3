...
function GetPtrType:onPtrOper1 (node)
   local cv = getCv (node:getCvSpecSeqOpt ())
...
function GetPtrType:onPtrOper3 (node)
   local cv = getCv (node:getCvSpecSeqOpt ())
   return PointerToMemberType (cv, self.to_tp, node:getObjName ())
end
...
function GetDcl:onDirectDcl1 (node)
   ...
   self.tp = FunctionType(cv, self.tp, getFuncParamTypes(node:getParamDecl1Body()), throw_spec)
...
function GetDcl:onDirectDcl4(node)
   -- function
   self.params = node:getParamDecl1Body()
   self.cv = getCv(node:getCvSpecSeqOpt())
...
function nodes.Decl:onNode ()
   return onDcl (getDeclSpec (self [1]), self [2])
end
